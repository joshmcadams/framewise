...
(11 chapters), author tutorial in `docs/tutorial.md`. The one idea everything hangs off:
**a video is a pure function of the frame number** — a composition asks "what frame are we
on?" and renders accordingly, identically in preview and export.
...
- Typecheck doc/tutorial snippets before publishing them: assemble every code example
  into a scratch `.tsx` under `src/`, run `npm run typecheck`, delete the file. This
  caught two broken examples pre-publication in `docs/tutorial.md` (a volume envelope
  that silently extrapolated past its range; an easing form that needed confirming).
...
## React/hooks traps (learned the hard way)

- Sync `setState` directly in an effect body trips `react-hooks/set-state-in-effect`.
  Prefer deriving state during render, or setting state only inside async callbacks,
  over disabling the rule — see `CompositionView`'s `settledText` pattern in
  `src/App.tsx` for the worked example.
- Never put freshly-created objects in effect dependency arrays (e.g. the result of
  `parsePropsInput`) — new identity per render means the effect loops forever. Depend
  on primitives and re-parse inside the effect body.
